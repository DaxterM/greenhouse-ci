...
if ((Get-Command "WDExpress.exe" -ErrorAction SilentlyContinue) -eq $null) {
  ...
  $p = Start-Process -FilePath "vs-express-setup.exe" -ArgumentList "/silent /passive /norestart /CustomInstallPath C:\Programs\VisualStudio" -Wait -PassThru

  if($p.ExitCode -ne 0) {
    Write-Error "Visual Studio Express 2013 installation process returned error"
  }
